#!/usr/bin/env python3
""" Main test module
"""
import requests
import re

URL = "http://localhost:5000"


def register_user(email: str, password: str) -> None:
    """ Test registering a user in the db
    """
    r = requests.post(URL + "/users", data={"email": email,
                                            "password": password})

    assert r.status_code == 200

    json = r.json()
    assert "email" in json
    assert json["email"] == email

    assert "message" in json
    assert json["message"] == "user created"


def log_in_wrong_password(email: str, password: str) -> None:
    """ Test logging in with the wrong password
    """
    r = requests.post(URL + "/sessions", data={"email": email,
                                               "password": password})

    assert r.status_code == 401


def log_in(email: str, password: str) -> str:
    """ Test logging in with the correct password
    """
    r = requests.post(URL + "/sessions", data={"email": email,
                                               "password": password})

    assert r.status_code == 200

    json = r.json()
    assert "email" in json
    assert json["email"] == email

    assert "message" in json
    assert json["message"] == "logged in"

    session_id = r.cookies.get("session_id")
    assert session_id is not None

    return session_id


def profile_unlogged() -> None:
    """ Test accessing the profile endpoint without being logged
    """
    r = requests.get(URL + "/profile")

    assert r.status_code == 403


def profile_logged(session_id: str) -> None:
    """ Test accessing the profile endpoint with a valid session
    """
    r = requests.get(URL + "/profile", cookies={"session_id": session_id})

    assert r.status_code == 200

    json = r.json()
    assert "email" in json


def log_out(session_id: str) -> None:
    """ Test logging out
    """
    r = requests.delete(URL + "/sessions", cookies={"session_id": session_id})

    assert r.status_code == 200
    assert r.url == URL + "/"
    assert len(r.history) > 0
    assert r.history[0].status_code == 302


def reset_password_token(email: str) -> str:
    """ Test get reset password token
    """
    r = requests.post(URL + "/reset_password", data={"email": email})

    assert r.status_code == 200

    json = r.json()
    assert "email" in json
    assert json["email"] == email

    assert "reset_token" in json

    return json["reset_token"]


def update_password(email: str, reset_token: str, new_password: str) -> None:
    """ Test updating a user password with their reset_token
    """
    r = requests.put(URL + "/reset_password", data={
        "email": email,
        "reset_token": reset_token,
        "new_password": new_password})

    assert r.status_code == 200

    # {"email": "<user email>", "message": "Password updated"}
    json = r.json()
    assert "email" in json
    assert json["email"] == email

    assert "message" in json
    assert json["message"] == "Password updated"


EMAIL = "guillaume@holberton.io"
PASSWD = "b4l0u"
NEW_PASSWD = "t4rt1fl3tt3"


if __name__ == "__main__":
    register_user(EMAIL, PASSWD)
    log_in_wrong_password(EMAIL, NEW_PASSWD)
    profile_unlogged()
    session_id = log_in(EMAIL, PASSWD)
    profile_logged(session_id)
    log_out(session_id)
    reset_token = reset_password_token(EMAIL)
    update_password(EMAIL, reset_token, NEW_PASSWD)
    log_in(EMAIL, NEW_PASSWD)
